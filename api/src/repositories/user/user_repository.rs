use crate::db::DB;
use crate::models::{IRegisterRequest, LoginRequest, RegisterRequest, User};
...
pub trait IUserRepository {
    ...
    async fn login(&self, user: LoginRequest) -> Result<User, UserError>;
    async fn update_by_id(&self, user: User) -> Result<User, UserError>;
...
impl IUserRepository for UserRepository {
    ...
    async fn login(&self, login: LoginRequest) -> Result<User, UserError> {
        let _exist = self.find_by_email(login.email.clone()).await;
        match _exist {
            Ok(user) => match login.check_user(user.clone()) {
                true => Ok(user),
                false => Err(UserError::WrongCredentials),
            },
            Err(_) => Err(UserError::UserAlreadyExist),
        }
    }
